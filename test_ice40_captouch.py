...
from migen import *
from random import *
from ice40_captouch import CapTouch
...
def testbench(e):
  ...
  for n in range(10):       # n captures
      ...
      for i in range(len(x_pads)):
          delay[i] = randrange(10)  # Wait for a random number of cycles to simulate captureA
      ...
      yield
      yield
      while ( yield e.i == (len(y_pads)-1) ):
          yield
      for i in range(len(x_pads)):
          yield e.source.data
      yield
      for i in range(len(x_pads)):
          yield e.lines_i[i].eq(0)
      yield
